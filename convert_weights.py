    ...
    for i, layer in enumerate(torch_model.modules):
        if layer.weight is not None:
            if block <= 5:
                ...
                block += 1
                if c == 3:
                    last_weights = torch.randn_like(var).double()
                    last_bias = torch.randn_like(var_bias).double()
                    layer.weight = last_weights
                    layer.bias = last_bias
                    #layer.weight = torch.cat((torch.tensor(layer.weight), last_weights), 0)
                    #layer.bias = torch.cat((torch.tensor(layer.bias), last_bias), 0)
                self_layer.weight.data[...] = torch.tensor(layer.weight).view_as(self_layer.weight)[...]
                self_layer.bias.data[...] = torch.tensor(layer.bias).view_as(self_layer.bias)[...]
                c+=1
    return model
