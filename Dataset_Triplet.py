...
class Dataset_Scrub_Triplet:
    ...
    def __getitem__(self, item):
        ...
        anchor_image_path = (
            self.path + "/" + self.labels[item] + "/" + anchor_image_name
        )
        ###### Anchor Image #######
        ...
        if self.is_train:
            ...
            positive_list = self.index[self.index != item][
                self.labels[self.index != item] == anchor_label
            ]
            positive_item = random.choice(positive_list)
            positive_image_name = self.images[positive_item]
            positive_image_path = (
                self.path + "/" + self.labels[positive_item] + "/" + positive_image_name
            )
            positive_img = Image.open(positive_image_path).convert("RGB")
            # positive_img = self.images[positive_item].reshape(28, 28, 1)
            negative_list = self.index[self.index != item][
                self.labels[self.index != item] != anchor_label
            ]
            negative_item = random.choice(negative_list)
            negative_image_name = self.images[negative_item]
            negative_image_path = (
                self.path + "/" + self.labels[negative_item] + "/" + negative_image_name
            )
