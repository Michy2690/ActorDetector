...
from DatasetTest import Dataset_Scrub_Triplet
...
class Trainer:
    ...
    def _save_checkpoint(self, epoch):
        with open("Weights_triplet_all/eval.txt", "a") as f:
            f.write("{}\nsum: {}\nmean: {}\n".format(epoch, np.sum(self.running_loss), np.mean(self.running_loss)))
            f.close()
        self.running_loss = []

    def train(self, max_epochs: int):
            for epoch in range(max_epochs):
                weights_path = 'Weights_triplet_all/checkpoint_{}.pth'.format(epoch)
            ...
            for epoch in range(15):
                weights_path = 'Weights_triplet_all_layers/checkpoint_{}.pth'.format(epoch)
                model_dict = torch.load(weights_path, map_location=lambda storage, loc: storage)
                self.model.load_state_dict(model_dict)
                self._run_epoch(epoch)
                if self.gpu_id == 0 and epoch % self.save_every == 0:
...
def load_train_objs():
    ...
    model_dict = torch.load('Weights_triplet_all/checkpoint_0.pth', map_location=lambda storage, loc: storage)
