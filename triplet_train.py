...
if __name__ == '__main__':
    ...
    model_dict = torch.load('Weights_triplet_augm/vggface_triplet_trained_0.pth', map_location=lambda storage, loc: storage)
    ...
    DATASET_PATH = "../Dataset_ridotto"

    train_data= pd.read_csv('dataset_ridotto.csv')
    ...
    for e in range(epochs):
        ...
        for i, (anchor_img, positive_img, negative_img, anchor_label) in enumerate(tqdm(train_loader, desc='Training', leave=False)):
            ...
            if i % 20 == 0:
                print(i)
        print('Epoch: {}/{} — Loss: {:.4f}'.format(e+1, epochs, np.mean(running_loss)))

        # Save trained weights
        model_file = os.path.join('./Weights_triplet_augm', 'vggface_triplet_trained_1.pth')
        ...
        with open("Weights_triplet_augm/ris.txt", "w") as f:
            f.write("\nmean: {}\nsum: {}".format(np.mean(running_loss), np.sum(running_loss)))
            f.close()
